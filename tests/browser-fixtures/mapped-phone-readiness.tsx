...
const leads=(sourceOutage?[1,2,3]:[1,2]).map(i=>({id:`mapped_fixture_${i}`,searchId:'mapped_fixture',name:`Вымышленная клиника ${i}`,
  ...
  contactCandidates:[{key:`phone:+99890123456${i}`,kind:'phone',phoneType:'mobile',value:`+99890123456${i}`,
    ownership:'company',lookupEligible:true,reason:'mobile_unverified',sourceUrl:i===3?'https://fixture.invalid/contacts':'https://www.openstreetmap.org/node/123456',evidenceIds:['fixture'],observedAt:'2026-03-07T00:00:00Z'}],
})) as LeadRadarLead[];
...
api.leadRadarCampaignPreflight=async ids=>{
  const verified=ids.filter(id=>id===(sourceOutage?'mapped_fixture_3':'mapped_fixture_2') && checked.has(id));
