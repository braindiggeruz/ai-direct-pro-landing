...
test('crawler source, migration, worker route and owner UI are preserved together', () => {
  ...
  assert.match(read('functions/api/lead-radar/crawler/[[path]].ts'), /CrawlerStore/);
  assert.match(read('functions/api/admin/lead-radar/[[path]].ts'), /handleCrawlerRequest/);
  assert.match(read('src/admin/pages/LeadRadar.tsx'), /WebsiteCollectorCard/);
  assert.match(read('wrangler.toml'), /LEAD_RADAR_CRAWLER_ENABLED = "true"/);
});

test('crawler extension stays independently pinned outside the stable research schema fingerprint', () => {
  const source = read('functions/platform/lead-radar/schema-contract.ts');
  const crawler = read('functions/platform/lead-radar/crawler.ts');
  assert.match(crawler, /CRAWLER_SCHEMA_FINGERPRINT/);
  assert.match(crawler, /crawlerSchemaFingerprint/);
  assert.match(source, /lead_radar_crawler_workers/);
  assert.match(source, /lead_radar_crawler_jobs/);
  assert.match(source, /type MigrationStage = 36 \| 41 \| 42 \| 43 \| 44;/);
  assert.doesNotMatch(source, /type MigrationStage =[^;]*56/);
});
