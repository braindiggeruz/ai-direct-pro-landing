...
export class LeadRadarTelegramBusinessStore {
  ...
  async findCompanyEvidenceForCampaignSelection(
    orgId: string,
    requests: ReadonlyArray<{ companyId: string; evidenceIds: readonly string[] }>,
  ): Promise<TelegramBusinessEvidenceRow[]> {
    if (requests.length === 0) return [];
    const result = await this.db.prepare(`SELECT evidence.id, evidence.org_id,
      evidence.company_id, evidence.field_path, evidence.value,
      evidence.source_url, evidence.source_type, evidence.observed_at,
      evidence.confidence, evidence.classification
    FROM json_each(?) request
    JOIN lead_radar_evidence evidence
      ON evidence.org_id = ?
        AND evidence.company_id = json_extract(request.value, '$.companyId')
        AND evidence.id IN (
          SELECT value FROM json_each(
            json_extract(request.value, '$.evidenceIds')
          ) WHERE type = 'text'
        )
    ORDER BY evidence.company_id, evidence.id`)
      .bind(JSON.stringify(requests), orgId)
      .all<TelegramBusinessEvidenceRow>();
    return result.results ?? [];
  }
