...
export function ImagePromptTool({ locale, onGenerate, disabled }: { locale: Locale; onGenerate: (prompt: string, presetId: string) => void; disabled?: boolean }) {
  ...
  return (
    // ym-disable-submit: the prompt generator's submission is user-authored
    // text; Form Analysis must not collect it.
    <form onSubmit={submit} className="rounded-2xl bg-brand-violet/[0.04] p-4 sm:p-5 ym-disable-submit">
      ...
      <label className="block mt-3 text-xs text-white/65">
        ...
        {/* ym-disable-keys: Webvisor must never record the typed idea. */}
        <textarea value={description} onChange={(event) => setDescription(event.target.value.slice(0, 800))} rows={3} placeholder={locale === 'uz' ? 'Masalan: Toshkentdagi qahvaxona uchun yozgi aksiya…' : 'Например: летняя акция кофейни в Ташкенте…'} className="mt-1.5 w-full rounded-xl border border-white/12 bg-[#0b101b] px-3 py-3 text-sm text-white placeholder:text-white/30 outline-none focus-visible:ring-2 focus-visible:ring-brand-cyan resize-y ym-disable-keys" />
      </label>
