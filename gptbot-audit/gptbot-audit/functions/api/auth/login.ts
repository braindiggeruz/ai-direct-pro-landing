...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  if (lockedFor > 0) {
    return json({ error: `Too many failed attempts. Try again in ${Math.ceil(lockedFor / 60)} min.`, lockedFor }, 429);
  }
  ...
  if (!ok) {
    ...
    if (r.lockedFor > 0) {
      return json({ error: `Locked for ${Math.ceil(r.lockedFor / 60)} min after ${r.count} failed attempts.`, lockedFor: r.lockedFor }, 429);
    }
    return json({ error: 'Invalid credentials', remaining: Math.max(0, 5 - r.count) }, 401);
  }
