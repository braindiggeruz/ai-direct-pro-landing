...
function slugify(text: string): string {
  return (text || '').toLowerCase().replace(/[^a-z0-9\u0400-\u04FF\u00C0-\u024F\u0100-\u017F ]/g, '').trim().replace(/\s+/g, '-');
}
...
function renderGptChatMain(
  ...
  <div class="mb-10">
    ...
    <div class="flex flex-col sm:flex-row sm:flex-wrap gap-3 mb-6">
      <a data-testid="page-cta-primary" href="${escapeHtml(primaryHref)}"${primaryExternal ? ' rel="nofollow noopener noreferrer" target="_blank"' : ''} class="btn-primary text-base w-full sm:w-auto">
        ${escapeText(primaryLabel)}
      </a>
      <a data-testid="page-cta-secondary" href="${escapeHtml(secondaryHref)}"${secondaryExternal ? ' rel="nofollow noopener noreferrer" target="_blank"' : ''} class="btn-secondary w-full sm:w-auto">
