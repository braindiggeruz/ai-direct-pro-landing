...
/**
 * Non-secret release sentinel for the stable routing key. The domain prefix
 * prevents this digest from being reused as a generic hash of the raw key.
 */
export async function routingKeyFingerprint(master: Uint8Array): Promise<string> {
  if (master.byteLength !== 32) throw new Error('routing_key_invalid');
  const domain = encoder.encode('gptbot-lead-radar-routing-key-fingerprint-v1\0');
  const framed = new Uint8Array(domain.byteLength + master.byteLength);
  framed.set(domain);
  framed.set(master, domain.byteLength);
...
/** Encrypts a mailbox payload under the gateway data key with exact metadata AAD. */
export async function encryptBridgePayload(input: {
  master: Uint8Array;
  scope: string;
  plaintext: string;
}): Promise<EncryptedBridgePayload> {
  const key = await hkdf(
    input.master,
    'gptbot-lead-radar-bridge-mailbox-v1',
    `mailbox:${input.scope}`,
    ['encrypt'],
    'AES-GCM',
  );
  ...
  const ciphertext = new Uint8Array(await crypto.subtle.encrypt({
    name: 'AES-GCM',
    iv,
    additionalData: encoder.encode(`bridge-mailbox:${input.scope}`),
    tagLength: 128,
...
export async function decryptBridgePayload(input: {
  master: Uint8Array;
  scope: string;
  envelope: EncryptedBridgePayload;
}): Promise<string> {
  if (input.envelope.version !== 1) throw new Error('bridge_payload_invalid');
  const iv = base64UrlToBytes(input.envelope.iv);
  const ciphertext = base64UrlToBytes(input.envelope.ciphertext);
  if (iv?.byteLength !== IV_BYTES || !ciphertext || ciphertext.byteLength < 17) {
  ...
  try {
    ...
    throw new Error('bridge_payload_decrypt_failed');
  }
}
