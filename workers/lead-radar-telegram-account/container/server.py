...
MAX_SANITIZED_MEDIA_BYTES = 10_000_000
MAX_SEND_BODY_BYTES = ((MAX_MEDIA_BYTES + 2) // 3) * 4 + 32_000
MAX_MEDIA_CAPTION_CHARACTERS = 1_024
MAX_IMAGE_DIMENSION_SUM = 10_000
MAX_IMAGE_ASPECT_RATIO = 20
...
EFFECT_RETENTION_SECONDS = 30 * 24 * 60 * 60
...
SAFE_MEDIA_ID = re.compile(r"^lrtgcm_[a-f0-9]{32}$")
MEDIA_MIME_FORMAT = {
    "image/jpeg": "JPEG",
    "image/png": "PNG",
    "image/webp": "WEBP",
}
MEDIA_VALIDATION_ONLY = os.environ.get("MEDIA_VALIDATION_ONLY") == "1"
VALIDATION_BOOT_ID = uuid.uuid4().hex

# A 4MP ceiling bounds an RGBA decode to about 16MiB and leaves headroom for
# the raw/base64 request, orientation, alpha flattening and JPEG encoding in
# the 128MiB container even when Pillow needs multiple pixel copies.
# Pillow's independent bomb warning is treated as a hard failure as well.
...
def decode_media_envelope(value: Any) -> tuple[bytes, str]:
    if not isinstance(value, dict) or not exact_keys(value, {
        "media_id", "media_digest", "mime_type", "size_bytes", "base64",
    }):
        raise MediaValidationError("media_shape_invalid")
    ...
    if not isinstance(media_id, str) or not SAFE_MEDIA_ID.fullmatch(media_id):
        raise MediaValidationError("media_id_invalid")
    if not isinstance(digest, str) or not SAFE_DIGEST.fullmatch(digest):
        raise MediaValidationError("media_digest_invalid")
    if mime_type not in MEDIA_MIME_FORMAT:
        raise MediaValidationError("media_type_invalid")
    if not isinstance(size_bytes, int) or isinstance(size_bytes, bool) \
            or not 1 <= size_bytes <= MAX_MEDIA_BYTES:
        raise MediaValidationError("media_size_invalid")
    if not isinstance(encoded, str) or len(encoded) % 4 != 0 \
            or len(encoded) > ((MAX_MEDIA_BYTES + 2) // 3) * 4:
        raise MediaValidationError("media_base64_invalid")
    try:
        raw = base64.b64decode(encoded, validate=True)
    except (ValueError, binascii.Error) as exc:
        raise MediaValidationError("media_base64_invalid") from exc
    if len(raw) != size_bytes \
            or base64.b64encode(raw).decode("ascii") != encoded \
            or hashlib.sha256(raw).hexdigest() != digest:
        raise MediaValidationError("media_digest_mismatch")
    signatures = {
    ...
    if not signatures[mime_type]:
        raise MediaValidationError("media_magic_invalid")
    return raw, mime_type


def sanitize_media_to_local_file(value: dict[str, Any]) -> tuple[Path, Path, int, int]:
    raw, mime_type = decode_media_envelope(value)
    expected_format = MEDIA_MIME_FORMAT[mime_type]
    temp_directory = Path(tempfile.mkdtemp(prefix="lead-radar-photo-", dir="/tmp"))
        ...
        with warnings.catch_warnings():
            ...
            with Image.open(io.BytesIO(raw)) as probe:
                if probe.format != expected_format \
                    ...
                    raise MediaValidationError("media_static_format_invalid")
                probe.verify()
            with Image.open(io.BytesIO(raw)) as decoded:
                ...
                if source_width < 1 or source_height < 1 \
                        ...
                        or source_width + source_height > MAX_IMAGE_DIMENSION_SUM \
                        or max(source_width, source_height) / min(source_width, source_height) \
                        > MAX_IMAGE_ASPECT_RATIO:
                    raise MediaValidationError("media_dimensions_invalid")
                decoded.seek(0)
                decoded.load()
                working_images: list[Image.Image] = []
                try:
                    oriented = ImageOps.exif_transpose(decoded)
                    if oriented is not decoded:
                        working_images.append(oriented)
                    width, height = oriented.size
                    if width < 1 or height < 1 \
                            or width * height > MAX_IMAGE_PIXELS \
                            or width + height > MAX_IMAGE_DIMENSION_SUM \
                            or max(width, height) / min(width, height) \
                            > MAX_IMAGE_ASPECT_RATIO:
                        raise MediaValidationError("media_dimensions_invalid")
                    # Pixel-only copy and JPEG re-encode strips EXIF/GPS, ICC,
                    # comments and container metadata. Alpha is flattened onto
                    # white so web mockups remain legible in Telegram.
                    if oriented.mode in ("RGBA", "LA") \
                            or (oriented.mode == "P" and "transparency" in oriented.info):
                        rgba = oriented.convert("RGBA")
                        working_images.append(rgba)
                        flattened = Image.new("RGB", rgba.size, (255, 255, 255))
                        working_images.append(flattened)
                        ...
                        sanitized = oriented.convert("RGB")
                        working_images.append(sanitized)
                    encoded: bytes | None = None
                    for quality, subsampling in ((92, 0), (85, 2), (75, 2)):
                        output = io.BytesIO()
                        sanitized.save(
                            output,
                            format="JPEG",
                        ...
                        if len(candidate) <= MAX_SANITIZED_MEDIA_BYTES:
                            encoded = candidate
                            break
                    if not encoded:
                        raise MediaValidationError("media_sanitized_too_large")
                finally:
                    closed: set[int] = set()
                    for image in reversed(working_images):
                        if id(image) not in closed:
                            image.close()
                            closed.add(id(image))
        path = temp_directory / "photo.jpg"
        descriptor = os.open(path, os.O_CREAT | os.O_EXCL | os.O_WRONLY, 0o600)
        with os.fdopen(descriptor, "wb") as output_file:
            output_file.write(encoded)
            output_file.flush()
            os.fsync(output_file.fileno())
        return temp_directory, path, width, height
    except Exception as exc:
        shutil.rmtree(temp_directory, ignore_errors=True)
        if isinstance(exc, MediaValidationError):
            raise
        raise MediaValidationError("media_decode_invalid") from exc


def validate_media_for_preapproval(value: dict[str, Any]) -> dict[str, Any]:
    """Authoritatively decode one image without touching TDLib or session state."""
    if not exact_keys(value, {"schema", "media"}) or value.get("schema") != SCHEMA:
        return response("error", reason_code="invalid_request")
    temp_directory: Path | None = None
    try:
        temp_directory, _path, _width, _height = sanitize_media_to_local_file(
            value.get("media")
        )
        return response("valid")
    except MediaValidationError:
        return rejected("media_invalid")
    finally:
        if temp_directory is not None:
            shutil.rmtree(temp_directory, ignore_errors=True)


def input_message_content(text: str, photo: tuple[Path, int, int] | None = None) -> dict[str, Any]:
    if photo is None:
        return {
    ...
    return {
        ...
        "caption": {"@type": "formattedText", "text": text, "entities": []},
...
class TdClient:
    ...
    def send_message(
        ...
        temp_directory: Path | None = None
        try:
            photo: tuple[Path, int, int] | None = None
            if media is not None:
                temp_directory, photo_path, width, height = sanitize_media_to_local_file(media)
                photo = (photo_path, width, height)
            ...
            sent = self.request({
                ...
                "reply_markup": None,
                "input_message_content": input_message_content(text, photo),
            }, 20.0)
            sending_state = sent.get("sending_state")
            if sending_state is None:
                return sent_message_result(sent)
            ...
            if early is not None:
                ...
                try:
                    ...
                    return ambiguous()
            return correlated_send_result(update)
        except TdError as error:
            return map_td_error(error.code, error.message)
        except TdTimeout:
            return ambiguous()
        except MediaValidationError:
            # Sanitization happens before sendMessage; this is a definitive
            # local rejection. The domain pauses this frozen campaign after
            # one attempt, releases its guard and compensates the quota.
            return rejected("media_invalid")
        finally:
...
def sent_message_result(message: Any) -> dict[str, Any]:
    """Accept only a TDLib message with a concrete integer provider id."""
...
def correlated_send_result(update: Any) -> dict[str, Any]:
    """Classify only explicit, structurally valid TDLib send updates."""
    if not isinstance(update, dict):
        return ambiguous()
    kind = update.get("@type")
    if kind == "updateMessageSendSucceeded":
        return sent_message_result(update.get("message"))
    ...
    if not isinstance(code, int) or isinstance(code, bool) \
        ...
        return ambiguous()
    return map_td_error(code, message)
...
class EffectLedger:
    ...
    def _prune_terminal_locked(self, now: int) -> int:
        assert self._connection is not None
        cursor = self._connection.execute(
            "DELETE FROM effects WHERE status IN ('sent','rejected','ambiguous') AND updated_at < ?",
            (now - EFFECT_RETENTION_SECONDS,),
        )
        return max(0, cursor.rowcount)

    def prepare_snapshot(self) -> int:
        """Prune expired terminal outcomes and compact before archive custody."""
        with self._lock:
            assert self._connection is not None
            self._connection.execute("BEGIN IMMEDIATE")
            removed = self._prune_terminal_locked(int(time.time()))
            self._connection.commit()
            # Never archive an uncheckpointed WAL. VACUUM bounds the physical
            # high-water mark after old terminal rows are deleted; in_flight
            # rows and every outcome inside the 30-day replay horizon remain.
            self._connection.execute("PRAGMA wal_checkpoint(TRUNCATE)")
...
class Runtime:
    ...
    def cancel_auth(self) -> bool:
        """Cancel only a pre-login TDLib authorization, atomically."""
        with self.lock:
            if self.client and self.client.authorization_state == "connected":
                return False
            self.reset(logout=False)
            return True


RUNTIME: Runtime | None = None if MEDIA_VALIDATION_ONLY else Runtime()
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _read_json(self, maximum_bytes: int = MAX_BODY_BYTES) -> dict[str, Any] | None:
        try:
    ...
    def do_GET(self) -> None:  # noqa: N802
        if self.path != "/v1/health":
            ...
            return
        state = (
            "validation_only" if RUNTIME is None
            else RUNTIME.client.authorization_state if RUNTIME.client
            else "not_started"
        )
        self._write_json(200, response(
            "ok",
            boot_id=RUNTIME.boot_id if RUNTIME is not None else VALIDATION_BOOT_ID,
            client_state=state,
            tdlib_source_commit=os.environ.get("TDLIB_SOURCE_COMMIT", "unconfigured"),
        ))

    def do_POST(self) -> None:  # noqa: N802
        if MEDIA_VALIDATION_ONLY and self.path != "/v1/media/validate":
            self._write_json(404, response("error", reason_code="not_found"))
            return
        ...
        value = self._read_json(
            MAX_SEND_BODY_BYTES
            if self.path in {"/v1/messages/send", "/v1/media/validate"}
            else MAX_BODY_BYTES
        )
    ...
    def _route(self, value: dict[str, Any]) -> dict[str, Any]:
        if self.path == "/v1/media/validate":
            return validate_media_for_preapproval(value)
        if RUNTIME is None:
        ...
        if self.path == "/v1/auth/cancel" and exact_keys(value, {"schema"}):
            return response("revoked") if RUNTIME.cancel_auth() else response(
                "error", reason_code="auth_state_conflict"
            )
    ...
    def _send(self, value: dict[str, Any]) -> dict[str, Any]:
        ...
        if not (exact_keys(value, base_keys) or exact_keys(value, base_keys | {"media"})):
            return response("error", reason_code="invalid_request")
        ...
        if not isinstance(username, str) or not SAFE_USERNAME.fullmatch(username):
            return response("error", reason_code="invalid_request")
        maximum_text = MAX_MEDIA_CAPTION_CHARACTERS if media is not None else 4_096
        if not isinstance(text, str) or not 1 <= len(text) <= maximum_text \
        ...
        if disposition == "replay" and stored is not None:
            return stored
        result = RUNTIME.ensure_client().send_message(username, text, operation_id, media)
