...
import { buildDirectWwwLegacyRules } from './redirect-rules';
...
const redirectedSources = new Set(userRedirects.map((redirect) => redirect.from));
const draftUrls = [
...
].filter((u) => u && u !== '/' && !redirectedSources.has(u));
...
lines.push('');
// 0) Collapse www + legacy-path combinations directly to their final target.
//    These specific rules must precede the generic www wildcard to avoid
//    www old path → apex old path → apex final target redirect chains.
const directWwwLegacyRules = buildDirectWwwLegacyRules(userRedirects);
if (directWwwLegacyRules.length) {
  lines.push('# 0) www legacy paths → final canonical targets (one hop)');
  lines.push(...directWwwLegacyRules);
  lines.push('');
}

// 0a) Canonical host: force every other www URL → non-www. 301 consolidates
//     ranking signals onto https://gptbot.uz.
...
console.log(`_redirects → dist/_redirects (${userRedirects.length} user redirects, ${draftUrls.length} draft 404s)`);
...
headers.push('/sitemap-priority.xml');
headers.push('  Cache-Control: public, max-age=300');
headers.push('  Content-Type: application/xml; charset=utf-8');
