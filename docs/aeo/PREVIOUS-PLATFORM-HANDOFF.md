...
> Source `9d3d2b7` is live on Pages deployment `09b442fd-a472-4e66-be52-fa199e7b02ca`; gateway 1.2.2 is deployment `bd5eae95-76fb-4b1d-af7f-79c51c0c416a` / Worker version `6aa02786-0a1a-4c9b-85d3-a857c9a63f6a`; Windows Bridge 1.2.0 and its DPAPI vault are preserved. HTTP, admin-boundary and SEO canaries pass. Phone and code delivery passed; the expired owner login must be repeated once to confirm 2FA completion, and the controlled-send canary remains pending.
...
> The following attempt accepted the Telegram code and reached 2FA, but the password never reached Telegram: its browser envelope inherited the ten-minute ceremony expiry and failed Bridge's 90-second per-input ceiling. Gateway also left that safe local failure unacknowledged, producing the apparent hang. Source `9d3d2b7` caps password envelopes at 60 seconds; gateway 1.2.2 ACKs the local failure, rotates a live password slot and closes an expired ceremony. The old provisional session was confirmed logged out. Production tests and live markers pass; one fresh owner phone/code/2FA flow is required before calling the account connected. No company message was sent.
